...
class LLamaQaStoppingCriteria(StoppingCriteria):
    """
    Reimplementation of the removed LLamaQaStoppingCriteria.
    Stops generation when any of the provided token-id sequences appear
    at the end of the generated output.
    """
    def __init__(self, list_token_ids_sequence):
        # list_token_ids_sequence: list of list of int
        self.token_ids_sequences = list_token_ids_sequence

    def __call__(self, input_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        for token_ids_sequence in self.token_ids_sequences:
            if len(token_ids_sequence) == 0:
                continue
            if input_ids.shape[1] < len(token_ids_sequence):
                continue
            if input_ids[0, -len(token_ids_sequence):].tolist() == token_ids_sequence:
                return True
        return False
...
class LLM:
    ...
    def generate(self, input_text, max_new_tokens=256, top_p=0.95, top_k=0, temperature=0.8, mode='vanilla', verbose=True, remove_stop_words=False, return_attentions=False, guiding_classifier=None, chunk_size=None, num_candidates=None, conversion_matrix=None, extra_prompt_length=None, teacher_forcing_seq=None, **kwargs):
        ...
        if not return_attentions:
            ...
            return output_str, outputs.attentions, gen_arr, outputs.scores